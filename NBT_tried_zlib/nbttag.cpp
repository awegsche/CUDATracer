...
std::string &NBTTag::Name()
{
    return _name;
}

void NBTTag::setName(const std::string &name)
{
    _name = name;
}


NBTTag* NBTTag::get_child(const std::string &name)
{
    return nullptr;
}
...
NBTTag *fromFile(BigEndianReader &r)
{
    ...
    case NBTTag::TAG_List:{
		...
		std::string name = std::string((char*)r.readBytes(length));
    ...
    case NBTTag::TAG_String:{
        ...
        tag->_value = std::string((char*)r.readBytes(stringlength));
        return tag;
    }
    default:
        break;
...
void assignName(NBTTag *tag, BigEndianReader &r)
{
    ...
    if(length == 0)
        ...
        byte* arr = r.readBytes(length);
		arr[length] = 0;
		std::string nme = std::string((char*)arr);
        tag->setName(nme);
    }
}
